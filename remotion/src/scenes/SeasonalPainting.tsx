...
export type PaintingLane = {
  /** Waypoints in the original image's native pixel coordinates. */
  start: [number, number];
  end: [number, number];
  /** Frames for one full pass along the lane. */
  period: number;
  /** Phase offset (frames) so lanes don't all start in sync. */
  offset?: number;
  /** Constant scale, or a [start, end] pair for a perspective (near/far) lane. */
  scale: number | [number, number];
  flip?: boolean;
  /** Constant opacity, or fade in/out at the lane's ends for a perspective lane. */
  fadeEnds?: boolean;
  baseOpacity?: number;
};

export type SeasonalPaintingProps = {
  image: string;
  nativeWidth: number;
  nativeHeight: number;
  lanes: PaintingLane[];
...
export function SeasonalPainting({
  image,
  nativeWidth,
  nativeHeight,
  lanes,
}: SeasonalPaintingProps) {
  ...
  // Blends an original-image point into the 1920x1080 frame, matching the
  // objectFit:"cover" scaling Img uses below.
  function mapPoint(ix: number, iy: number) {
    const scale = 1080 / nativeHeight;
    const offsetX = (1920 - nativeWidth * scale) / 2;
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: "#000" }}>
      ...
      >
        {lanes.map((lane, i) => {
          const period = lane.period;
          const t = (((frame + (lane.offset ?? 0)) % period) + period) % period / period;
          const start = mapPoint(lane.start[0], lane.start[1]);
          const end = mapPoint(lane.end[0], lane.end[1]);
          const x = interpolate(t, [0, 1], [start.x, end.x]);
          const y = interpolate(t, [0, 1], [start.y, end.y]);
          const scale = Array.isArray(lane.scale)
            ? interpolate(t, [0, 1], lane.scale)
            : lane.scale;
          const opacity = lane.fadeEnds
            ? interpolate(t, [0, 0.08, 0.85, 1], [0, lane.baseOpacity ?? 0.5, lane.baseOpacity ?? 0.5, 0])
            : lane.baseOpacity ?? 0.5;
          return (
            <CarBlob
              key={i}
              x={x}
